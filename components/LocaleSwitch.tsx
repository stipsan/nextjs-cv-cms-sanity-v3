...
export default function LocaleSwitch() {
  ...
  return (
      ...
      <nav className="shadow-inner p-0.5 rounded-lg justify-self-center inline-flex space-x-1 bg-slate-100 transition">
        {locales.map((link) => (
          <Link key={link} href={route} locale={link} prefetch={false}>
            <a
              className={cx(
                ...
                {
                  'text-slate-600 bg-white shadow pointer-events-none':
                    link === locale,
