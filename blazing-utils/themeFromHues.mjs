...
function hslToRgb(hue, saturation, lightness, convert) {
  ...
  huePrime >= 0 && huePrime < 1
    ...
    : huePrime >= 5 &&
      huePrime < 6 &&
      ((red3 = chroma), (blue3 = secondComponent))
...
function rgbToHsl(color) {
  var red3 = color.red / 255,
    green3 = color.green / 255,
    blue3 = color.blue / 255,
    max = Math.max(red3, green3, blue3),
    min = Math.min(red3, green3, blue3),
    lightness = (max + min) / 2
  if (max === min)
...
function convertToHex(red3, green3, blue3) {
  return reduceHexValue$1(
    '#' + colorToHex(red3) + colorToHex(green3) + colorToHex(blue3)
  )
}
...
function rgb(value, green3, blue3) {
  if (
    typeof value == 'number' &&
    typeof green3 == 'number' &&
    typeof blue3 == 'number'
  )
    return reduceHexValue$1(
      '#' + numberToHex(value) + numberToHex(green3) + numberToHex(blue3)
    )
...
var isRgb = function (color) {
    return (
      ...
      (typeof color.alpha != 'number' || typeof color.alpha > 'u')
    )
  },
...
function themeFromHues({
  ...
  let hues2 = applyHues(partialHues),
    black3 = { title: 'Black', hex: hues2.default.darkest },
    white3 = { title: 'white', hex: hues2.default.lightest },
    tones2 = createTonesFromHues(hues2),
    ...
    color = createColorTheme2({
      base: ({ dark, name }) => {
        if (name === 'default') {
          let skeletonFrom2 = dark
            ? tones2.transparent[900].hex
            : tones2.transparent[100].hex
          return {
            ...
            shadow: {
              ...
              umbra: rgba3(
                dark ? black3.hex : tones2.transparent[500].hex,
                0.2
              ),
              penumbra: rgba3(
                dark ? black3.hex : tones2.transparent[500].hex,
      ...
      button: ({ base, mode, muted, solid }) =>
        mode === 'bleed'
          ...
          : mode === 'ghost'
          ? __spreadProps(__spreadValues({}, solid), {
              enabled: __spreadProps(__spreadValues({}, muted.enabled), {
                border: base.border,
              }),
      ...
      selectable: ({ base, muted, tone, solid, state }) =>
        state === 'enabled'
          ? __spreadProps(__spreadValues({}, muted[tone].enabled), {
              bg: base.bg,
            })
          : state === 'pressed'
          ? tone === 'default'
          ...
          ? tone === 'default'
            ...
            : solid[tone].enabled
          : state === 'disabled'
          ? __spreadProps(__spreadValues({}, muted[tone].disabled), {
              bg: base.bg,
            })
          : muted[tone][state],
      spot: ({ base, dark, key }) =>
        (dark ? screen3 : multiply3)(
          base.bg,
          getTint(key)[dark ? 400 : 500].hex
        ),
      syntax: ({ base, dark }) => {
        let mix2 = dark ? screen3 : multiply3,
...
var black2 = 'hsl(0, 0%, 0%)',
  ...
  defaultOpts = {
    ...
    button: ({ base, mode, muted, solid }) =>
      mode === 'bleed'
        ? __spreadProps(__spreadValues({}, muted), {
            enabled: {
              bg: 'transparent',
              bg2: 'transparent',
              fg: muted.enabled.fg,
...
function createSelectableTones(opts, base, dark, solid, muted) {
  return {
    ...
    positive: _createSelectableStates(
      opts,
      base,
      dark,
      solid,
    ...
    critical: _createSelectableStates(
      opts,
      base,
      dark,
      solid,
